...
948066))
chromosome_lengths
# information from the SGD and associated websites
```
...
ChromoStartEnd=GFFAnno %>%
  group_by(chromosome)
# groups by chromosome. GFFAnno is already grouped by chromosome, but let's do this JUST IN CASE there are out-of-place regions.
# For some reason this doesn't work on merged_data...
  # Not sure why, but I did not have enough time to figure out why within my original project deadline
tail(ChromoStartEnd)
```
...
Within the three special regions, a high amount of genes exhibit notably low expression at the start of the time course. This makes sense, as the meiosis has just begun. The lack of expression in the beginning of meiosis suggests the genes in the three regions are not master regulators.
...
UPDATED NOTES JUNE 2024:
Additionally, there appear to be some outlier genes with massive underexpression (point D and I)
It should also be noted that in order to properly fit the datapoints in the screen, the plot for control required a much smaller bin width (0.017) compared to the 3 regions (0.05). Because of this, it can be concluded that there was an intensely larger amount of genes in the control across all patterns and time points compared to the 3 regions of interest.
...
# Avg exp lvl of individual regions
...
ViolinPeri
```
The pericentromeric region takes up the majority of the combined 3 regions' avg gene exp. It largely follows the same pattern of the combined plot, with minimal differences in gene amount (plot width)
...
ViolinSub
```
The subtelomeric region avg gene exps are very sparse compared to those of the pericentromeric region. The rising-falling expression pattern is largely the same, but most of the genes accumulate at a greater underexpression (more negative number, shifted downwards) than that of the combined plot. This can be seen prominently on time points A and I. The contribution of the subtelomeric regions' avg gene exp on the combined plot are suspected to be minimal.
...
ViolinRibo
```
Like the subtelomeric region, the ribosomal region avg gene exps are very sparse and likely contributes very little to the combined region plot. Curiously, the shifts in largest accumulations are shifted upwards (greater expressions). (Visible on time points A, B, and I)
This is the opposite compared to the subtelomeric regions. I suspect that the subtelomeric and ribosomal regions' effects on the combined 3 regions' avg gene exp cancel each other out somehow.
